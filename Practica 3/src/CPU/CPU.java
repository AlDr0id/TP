...
public class CPU {
	 ...
	 * @throws StackException */
	public boolean read(int param) throws StackException {
	...
	public boolean run() throws DivisionByZero, StackException { // Si no hay error devuelve false
		...
		while (this.programCounter < bcProgram.getIndice() && !error && !this.terminado) {
			...
			try {
				...
				throw new DivisionByZero("Has dividido entre 0 en la linea " + this.programCounter);
		
			}
			catch (StackException e) {
				throw new StackException(e.getMessage() + "en la linea " + this.programCounter);
			}
		}
	...
	public boolean out() throws StackException {
		...
		if (this.pila.noVacia()) {
			System.out.println("Cima de la pila: " + this.pila.getCima() 
